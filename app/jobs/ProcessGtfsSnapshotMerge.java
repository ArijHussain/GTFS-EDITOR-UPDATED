...
public class ProcessGtfsSnapshotMerge extends Job {
	...
	public void doJob(File gtfsUpload){
    	...
    	try {
			...
			/*File file2 = new File(snapshotMerge.snapshot.getFilename());

			System.out.println("name %%%%%%%%%%%%%"+gtfsUpload.getName());
			File source = new File(gtfsUpload.getParentFile().toString());
			File dest = new File(Play.configuration.getProperty("application.publicGtfsDataDirectory"));
			try {
			...
			System.out.println("gtfsFile***********"+gtfsFile);
